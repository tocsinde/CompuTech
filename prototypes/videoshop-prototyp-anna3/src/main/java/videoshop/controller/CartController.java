...
import videoshop.model.SaleOrder;
	...
	public String buy(@ModelAttribute Cart cart, @LoggedIn Optional<UserAccount> userAccount) {
		...
		return userAccount.map(account -> {
			...
			// Orders können nur abgeschlossen werden, wenn diese vorher bezahlt wurden.
				...
				SaleOrder order = new SaleOrder(account, Cash.CASH);
			
				cart.addItemsTo(order);
				int quantity = 0;
				for (OrderLine test: order.getOrderLines()){
					quantity+=test.getQuantity().getAmount().intValue() ;
				}
				if (quantity >= BossController.quantity)
					order.isSale = true;
